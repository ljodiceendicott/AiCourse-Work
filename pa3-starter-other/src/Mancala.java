...
public class Mancala implements Game {
        ...
        public void makeMove(char player, int move) {
            ...
            do {
                for(int i = move; i < board.length; i++){
                    ...
                    if(player == 'p' && i+1 == 13 && numpel > 0){
                        i = -1;
                        board[0]++;
                        numpel--;
                        continue;
                    }
                    else if(player == 'c' && i+1 == 6 && numpel > 0){
                        continue;
                    }
                    else{
                        board[i+1]++;
                        numpel--;
                        if(numpel == 0 && board[i+1] > 1 && i+1 != 13 && i+1 != 6){
                            numpel = board[i+1];
                            board[i+1] = 0;
                            move = i;
                            continue;
     ...
     */
    public Mancala(int loggingDepth, int depth) {
    ...
    public void run() {
        ...
        while (true) {
           ...
           currentState.makeMove('p', userMove);
           printBoard(currentState.board);
           ...
           // Computer's turn.
           System.out.println("\nComputer's turn:");
           // currentState = new MancalaState(currentState.board, 'c',userMove);
           currentState.move=userMove;
           MancalaActionUtility s = (MancalaActionUtility)minimax.value(currentState, depth, loggingDepth, "");
           aiMove = s.move;
           currentState.makeMove('c', aiMove);
  ...
  board[6]+"                                                   "+board[13]+"\n"+
    ...
    public static void main(String[] args) {
        ...
        Mancala mancala = new Mancala(loggingDepth, depth);
        mancala.run();
    }
