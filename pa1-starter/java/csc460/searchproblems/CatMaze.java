...
public class CatMaze implements SearchProblem {
    ...
    public CatMaze() {
        ...
        colorMap.put('h', Color.CYAN);
        colorMap.put('m', Color.DARK_GRAY);
    }
    ...
    public boolean isGoal(SearchState state) {
        if(mice.isEmpty()){
            ...
            for(int i =0; i<mice.size(); i++){
                if(state.getAgentCoordinates()==mice.get(i)){
                    mice.remove(i);
                }
            }
            return false;
    ...
    public double getDistance(BoardCoordinate coord){
        if(mice.isEmpty()){
            ...
            for(int i=0; i<mice.size(); i++){
                int distance=Math.abs(coord.x - mice.get(i).x) + Math.abs(coord.y - mice.get(i).y);
               if(distance<closest){
                   closest=distance;
                   }
            }
    ...
    public Iterable<SearchState> getSuccessors(SearchState state) {
        ...
        for(int i = 0; i < actions.length; i++){
            //if the end was found but there was still a mouse on the board dont do that action. 
            if(successorCoords[i].equals(exit)&&mice.size()>1){
                break;
            }
            ...
            if(successorCoords[i].y >= 0 && 
                ...
                internalBoard.get(successorCoords[i].y)
                    ...
                    internalBoard.get(successorCoords[i].y).get(successorCoords[i].x) == 'h'
                    ){
                  successors.add(new MazeState(successorCoords[i], actions[i],5, getDistance(successorCoords[i])));   
                    }
            else if(successorCoords[i].y >= 0 && 
                successorCoords[i].y < internalBoard.size() &&
            successorCoords[i].x >= 0 && 
                successorCoords[i].x < internalBoard.get(0).size() && 
            internalBoard.get(successorCoords[i].y)
                .get(successorCoords[i].x) != 'w'){
                successors.add(new MazeState(successorCoords[i], actions[i],1, getDistance(successorCoords[i])));
            }
        }
     ...
     *   m -- mouse (item that needs to be picked up before reaching the end)
     *   h -- hazard (space that can be moved into but has a high cost associated with it)
    ...
    public Board loadBoardFile(String filename) throws FileNotFoundException {
        ...
        int mapSize = Integer.valueOf(reader.nextLine());
        ...
        startState = new MazeState(start, "", 0.0, getDistance(start));
